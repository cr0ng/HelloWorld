...
void ShipManager::PlaceAllShips(HiddenMap& InMap)
{
	...
	for (int i = 0; i < ShipTypeCount; i++)
	{
		...
		while (!IsPlaced) // 배치가 완료될 때까지 while 반복
		{
			...
			{
				// 배치할 수 있으면 배치 처리
				Position PlacePosition[ShipTC::MaxShipSize];
				for (int j = 0; j < ShipSizes[i]; i++)
				{
					int NewX = x + (IsHorizontal ? i : 0);
					int NewY = y + (IsHorizontal ? 0 : j);
					InMap.SetCellType(NewX, NewY, CellType::Ship);	// 맵에 기로갛고
...
bool ShipManager::CanPlaceShip(int InX, int InY, int InSize, bool InHorizontal, const HiddenMap& InMap) const
{
	for (int i = 0; i < InSize; i++)
	{
		...
		{
			return false;	// 해당 셀이 비어있찌 않으면 배치 불가
		}
	}
	return false;	// 모든 칸이 확인이 끝났고 맵 안이고 셀이 비어있다는게 확인이 되었으니 배치 가능
}
