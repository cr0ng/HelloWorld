...
public:
		...
		: MaxSize(InSize == 0 ? MinSize :InSize)
	{
		if (InSize == 0)
		{
			printf("요청한 크기 0은 유효하지 않으므로 최소 크기인 1로 설정했습니다.");
		}
	...
	// 순수하게 테스트용. 큐 내부를 모두 출력하는 함수
	void TestPrintQueue();

	// 큐가 가득 찼는지 확인하는 함수
	inline bool IsFull() const
	{
		// Rear가 하나더 증가했을때 Front를 침버하냐 마냐로 가득참을 체크
	...
	{
		return Front == Empty; // Front가 배열 내 아무곳도 가리키지 않으면 비었다고 판단
	}
...
private:
	...
	static constexpr int unsigned MinSize = 1;
